...
import numpy as np
import src.engine.utils.accuracy as acc
import src.engine.utils.histories as history
import tensorflow.keras as keras
from matplotlib import pyplot as plt
from src.engine.network import Network
from src.engine.network.activations import Activations
from src.engine.network.optimizers import Optimizers
from src.engine.utils.accuracy import r_squared
...
class MLP(Network):
    ...
    def __init__(self, run_num=101, multi_run=False, plot=True, check=False, max_epoch=5000, min_accuracy=0.99,
        ...
        file = open(self.project_path + '/architecture.text', 'w+')
        file.write('')
        file.close()
    ...
    def set_data(self, data):
        ...
        np.savetxt(self.working_directory + '/.temp/normalized_training_data.text',
    ...
    def add_layer(self, neuron, activation_function=Activations.linear, kernel_initializer='uniform',
                  layer_type=Network.HIDDEN_LAYER, kernel_regularizer=None):

        file = open(self.project_path + '/architecture.text', 'a')

        # add an input layer -------------------------------------------------------------------------------------------
        if layer_type == Network.INPUT_LAYER:
        ...
        # add an output layer ------------------------------------------------------------------------------------------
        elif layer_type == Network.OUTPUT_LAYER:
            self.model.add(Dense(neuron, activation=activation_function, kernel_initializer=kernel_initializer,
    ...
    # compile network ==================================================================================================
    def compile(self, optimizer=Optimizers.Adam, loss=keras.losses.mse, metrics=None):
    ...
    def exe(self, multi_processing=True, verbose=1, batch_split=0.2):
        ...
        # setting history
        histories = history.Histories(max_loss=self.max_loss, min_accuracy=self.min_accuracy,
        ...
        result_file_name = self.working_directory + "/result.text"
    ...
    def prediction(self):
        ...
        self.final_training_accuracy = acc.r_squared_np(self.y_true_training, self.y_prediction_training)
        ...
        self.final_test_accuracy = acc.r_squared_np(self.y_true_test, self.y_prediction_test)
        ...
        self.final_all_accuracy = acc.r_squared_np(self.y_true_all, self.y_prediction_all)
    ...
    def trained_model_prediction(self, file_path, input_data, output_data):
        ...
        # saving Evaluation of training results ------------------------------------------------------------------------
        result_file_name = working_dir + "/regression.text"
